...
app.post("/split-payments/compute", computeFeeValidation(), async(req, res) => {
 
  const { ID, Amount, SplitInfo } = req.body

  let final_split_results = []
  console.time("response_timer")
    ...
    if (splitInfo.SplitType.toLowerCase() === "flat") {
      ...
      if (amount > Number(Amount)) {
        return res.status(400).json({
          status: "NOT OK",
          message: "Check that you entered the correct data as the computed amount should not be greater than the transactional amount",
          response: splitInfo
        })
      }
  ...
  console.timeEnd("response_timer")
